...
class NavigationTest(BaseCase):
    ...
    def test_03_find_items(self):
        ...
        # получаем  все элементы по заданному х-пазу и считаем длину списка
    ...
    def test_05_change_region(self):
        ...
        # задаем константы
        text_path = '//article[1]/a/div[2]/p[2]'
        price_path = '//*[@id="products"]/article[1]/a/div[2]/p[3]/span[2]'

        # получаем английский текст
        eng_text = self.get_text(text_path)
        # получаем стоимость в евро до смены языка
        eu_price = self.get_text(price_path)
        ...
        ru_text = self.get_text(text_path)
        # получаем стоимость в $
        usd_price = self.get_text(price_path)
        # название товара на английском и русском языках совпадает 
        self.assert_equal(eng_text, ru_text)
        # цены не должны совпадать
        self.assert_not_equal(eu_price, usd_price)
